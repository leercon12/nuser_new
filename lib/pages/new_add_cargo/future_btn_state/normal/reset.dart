...
Future<void> resetNormal(
  ...
  try {
    ...
    if (callType == '수정') {
      ...
      await liveRef.set(liveData, SetOptions(merge: true));
    }

    if (callType.contains('기사업데이트')) {
      await checkAndUpdateUpDownDocs(
          cargo['pickUserUid'], cargoId, cargo, addProvider, hashProvider);
    }
    ...
    ScaffoldMessenger.of(context)
        .showSnackBar(currentSnackBar('운송 정보가 정상적으로 수정되었습니다.', context));
...
Future<void> checkAndUpdateUpDownDocs(
  String pickUserUid,
  String cargoId,
  Map<String, dynamic> cargo,
  AddProvider addProvider,
  ...
  try {
    // 상차지 문서 업데이트
    final upDocRef = FirebaseFirestore.instance
        .collection('user_driver_cargo')
        .doc(pickUserUid)
        .collection('cargo_info')
        .doc('${cargoId}up');

    final upDocSnapshot = await upDocRef.get();

    if (upDocSnapshot.exists) {
      await upDocRef.update({
        'type': 'up',
        'id': cargoId + 'up',
        'listId': cargoId + 'up',
        'cargoId': cargoId,
        ...
        'isDone': false,
        'uid': cargo['uid'],
        'ownerType': cargo['ownerType'],
        'price': cargo['payMoney'],
        'address': addProvider.setLocationUpAddress1,
        'disAds': addProvider.setLocationUpDis,
        'name': addProvider.setLocationUpName,
        'upPhone': addProvider.setLocationUpPhone,
        'comUid': cargo['comUid'],
    ...
    // 하차지 문서 업데이트
    final downDocRef = FirebaseFirestore.instance
        .collection('user_driver_cargo')
        .doc(pickUserUid)
        .collection('cargo_info')
        .doc('${cargoId}down');

    final downDocSnapshot = await downDocRef.get();

    if (downDocSnapshot.exists) {
      await downDocRef.update({
        'type': 'down',
        'id': cargoId + 'down',
        'listId': cargoId + 'down',
        'cargoId': cargoId,
        'transitType': '일반',
        'confrimeDate': cargo['fixDate'],
        'upDate': addProvider.setDownDate,
        'upLocation': {
          'geohash': null,
          'geopoint': await hashProvider
              .encryptNLatLng(addProvider.setLocationUpNLatLng as NLatLng)
        ...
        'ownerType': cargo['ownerType'],
        'price': cargo['payMoney'],
        'address': addProvider.setLocationDownAddress1,
        'disAds': addProvider.setLocationDownDis,
        'name': addProvider.setLocationDownName,
        'upPhone': addProvider.setLocationDownPhone,
        'comUid': cargo['comUid'],
        'payType': cargo['payType'],
        'senderType': addProvider.senderType,
        'isNormal': cargo['isNormal'],
    ...
    throw Exception('상/하차지 문서 업데이트 실패: $e');
  }
}
