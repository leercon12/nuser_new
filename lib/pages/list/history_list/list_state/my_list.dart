...
class HistoryMyList extends StatefulWidget {
  const HistoryMyList({super.key});

  @override
  State<HistoryMyList> createState() => _HistoryMyListState();
}

class _HistoryMyListState extends State<HistoryMyList> {
  ...
  String selectedField = 'upAddress'; // 기본값: 상차지 주소
  int? selectedMonth; // 선택된 월 (null이면 전체 월)
  int? selectedDay; // 선택된 일 (null이면 전체 일)

  // 검색 필드 옵션
  final List<Map<String, String>> searchFields = [
    {'value': 'upAddress', 'label': '상차지 주소'},
    {'value': 'downAddress', 'label': '하차지 주소'},
    {'value': 'upName', 'label': '상차지명'},
    {'value': 'upPhone', 'label': '상차지 번호'},
    {'value': 'downName', 'label': '하차지명'},
    {'value': 'downPhone', 'label': '하차지 번호'},
    {'value': 'driverName', 'label': '기사 이름'},
    {'value': 'driverPhone', 'label': '기사 전화번호'},
    {'value': 'comName', 'label': '주선사 이름'},
    {'value': 'comPhone', 'label': '주선사 번호'},
  ];
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      print('데이터 로드 - 연도: $selectedYear, 사용자: ${dataProvider.uid}');
  ...
  void _handleFieldChange(String newField) {
    setState(() {
      selectedField = newField;
      // 필드만 변경하고, 데이터는 자동으로 다시 로드하지 않음
    });
  }
    ...
    return Column(
      children: [
        ...
        Padding(
          ...
          child: Column(
            children: [
              // 첫 번째 줄: 텍스트 필드와 검색 버튼을 포함한 검색 바

              // 두 번째 줄: 연/월/일 선택 드롭다운
              Row(
                children: [
                  // 연도 선택 드롭다운
                  Expanded(
