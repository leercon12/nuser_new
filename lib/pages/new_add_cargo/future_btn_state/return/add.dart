      ...
      try {
        ...
        if (cargo.upCargos != null && cargo.upCargos.isNotEmpty) {
          ...
          for (var cargoInfo in cargo.upCargos) {
            ...
            print('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@');
            print(cargoInfo.type);
            print(processedInfo['type']);
...
// 모든 위치에서 downCargos 처리
        ...
        for (var i = 0; i < locations.length; i++) {
          print('Location $i:');
          print('- type: ${locations[i].type}');
          print('- address1: ${locations[i].address1}');
    ...
    final cargoData = {
      ...
      'sanjeahalf': null,
      'vat': null,
      'downAloneType': processedLocations.first['dateAloneString'],
      'upAloneType': processedLocations.last['dateAloneString'],
      'pickUserUid': null,
      'pricePerKm': null,
          ...
          addProvider.senderUpType == true && addProvider.senderDownType == true
              ...
              : addProvider.senderUpType == true
                  ...
                  : addProvider.senderDownType == true
                      ? '하차지와 동일'
                      : null, */
      'clientInfo': addProvider.senderType == '일반'
          ? '${dataProvider.name} / ${dataProvider.phone}'
          : '${dataProvider.comName} / ${dataProvider.phone}',
      'comUser': null,
      'comUserName': null,
    ...
    final liveData = {
      ...
      'upComType': addProvider.locations.first.senderType,
      'downComType': addProvider.locations.last.senderType,
      'transitType': addProvider.addSubType,
      'aloneType': addProvider.addMainType,
      'normalEtax': addProvider.isNormalEtax,
      'multiNum': totalLocations,
